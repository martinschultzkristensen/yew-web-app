...
pub fn video_import(props: &ImageImportProps) -> Html {
    ...
    let on_import_click = {
        ...
        Callback::from(move |_| {
            ...
            spawn_local(async move {
                // Open file dialog
                let select_result = invoke("select_img_file", JsValue::NULL).await;
                ...
                match serde_wasm_bindgen::from_value::<String>(select_result) {
                    Ok(selected_path) => {
                        ...
                        let import_result = invoke("import_images", js_args).await;
    ...
    html! {
        <button onclick={on_import_click} disabled={*importing}>
            { if *importing { "Importing..." } else { "Import image" } }
        </button>
    }
}